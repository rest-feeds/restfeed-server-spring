...
import org.restfeeds.server.spring.JdbcFeedItemRepository;
...
class RestFeedServerSpringApplicationTests {
  ...
  void shouldReturnItemFromOffset() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.get("/movies?offset=2"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("[0].id").value("cd5fadad-7c05-47c9-8d66-14b81588d445"))
        .andExpect(jsonPath("[0].next").value("http://localhost/movies?offset=3"))
        .andExpect(jsonPath("[1]").doesNotExist());
  }
  ...
  void nextLinkShouldRespectXForwardedProtoHeader() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.get("/movies").header("X-Forwarded-Proto", "https"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("[0].next").value("https://localhost/movies?offset=2"));
  }
